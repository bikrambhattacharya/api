class IsolateJob < ApplicationJob
  ...
  def compile
    ...
    submission.compile_output = Document.find_or_create_with_content(`cd #{box} && timeout -s 15 -k 5s 10s #{submission.language.compile_cmd} 2>&1`.presence)

    process_status = $?
    return :success if process_status.success?

    if [124, 137].include? process_status.exitstatus
      submission.compile_output = Document.find_or_create_with_content("Compilation time limit exceeded.")
    end
